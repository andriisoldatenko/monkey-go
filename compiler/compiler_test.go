...
func testConstants(t *testing.T, exppected []any, actual []object.Object) error {
	if len(exppected) != len(actual) {
		return fmt.Errorf("wrong number of constants. got=%d, want=%d", len(actual), len(exppected))
	}

	for i, constant := range exppected {
